...
China's 98% share of low-purity gallium sat in USGS yearbooks for a decade — not classified, not obscure, just unread (USGS MCS 2023). In the twenty-four months before the July 2023 export controls, English-language news mentioned gallium in a supply-chain context 394 times, out of 6.8 million articles (my measurement). The day after the announcement, the U.S. substrate maker AXT was filing for Chinese export permits (Reuters, July 4, 2023). Neon in 2022 was the same story: half of semiconductor-grade supply ran through two purification firms in Mariupol and Odesa — findable by anyone, found by no one until the missiles landed (Reuters, March 11, 2022).
...
This paper works with three axes and treats each differently. Criticality is borrowed rather than rebuilt: the numbers are other people's, published before any of these controls existed. Visibility is measured directly, node by node, across a full commodity pool. Reflexivity is not a variable to be scored but a property of the thing being predicted. Some facts move the moment people learn them — prices, tariffs, anything with a trader on the other side. Others do not, because they were laid down by history and cannot be relaid: a refinery's gallium line, once installed, is not undone by an article about it. The second kind is what this framework predicts. No single axis does the work. The USGS supply-risk score — published for 49 of these commodities — puts two of the later-restricted ones in its top ten. Invisibility alone, ranked across the full scorable pool, puts four: a list full of thallium and kyanite. Multiply invisibility by criticality and the same pool yields nine, restricted twenty-one to twenty-seven months after the map was drawn.

Together the three axes define a state space: eight regions, four of them high-criticality. Power sleeps in the **latent zone** — critical, unwatched, structural — next door to the **landmark zone** (already priced in), the **arena** (prices and tariffs, reflexive by nature), and the **dark currents**. Each has a low-criticality shadow. The **dust corner** — thallium, kyanite — is where you land if you rank by obscurity alone: nobody watches these because nobody needs them. Naming that space, measuring the axis that locates the latent zone inside it, and grading the result against six reveals is what this paper contributes: a map of where latent chokepoints sit, not a forecast of who Beijing restricts next.
...
**H1 · Mechanism** (why the latent zone exists) → **H2a · Map test** (does the assessment hold against reality? six control rounds 2023–25 as grading events — validation, not policy forecasting) → **H2b · Awakening test** (among the restricted, do more latent ones hit harder? strategy conditioned away).
...
**Author datasets (this repository):** three per-hypothesis workbooks in `data/` — `H1_mechanism_data_2026-08.xlsx`, `H2a_map_data_2026-08.xlsx` (sheet `1_Results` holds all final numbers), `H2b_awakening_data_2026-08.xlsx` — each sheet carrying its source, with `docs/03_data_readme.md`; flat CSVs in `data/sheets_csv/`.
